...
const SidebarLeft = ({ isMobile, leftBar }) => {
                        ...
                        try {
                            if (item.type === "artist") {
                                const res = await fetch(
                                    `http://localhost:8000/api/music/artist/${item.id}`
                                );
                                ...
                                const res = await fetch(
                                    `http://localhost:8000/api/music/album/${item.id}`
                                );
                            ...
                            console.error(
                                `Failed to fetch ${item.type} info for ID ${item.id}`,
                                err
                            );
                        }
    ...
    const filteredComponents = components
        .filter((item) => {
            const bool1 = item.name
                .toLowerCase()
                .includes(searchTerm.toLowerCase());
            const bool2 = filter
                ? filter === "album"
                    ? item.type.toLowerCase().includes("single") ||
                      item.type.toLowerCase().includes("composite")
    ...
    const handleCreatePlaylist = async () => {
        ...
        try {
            ...
            const userId = user?.id;
            const res = await authFetch(
                `http://localhost:8000/api/music/user/${userId}/create_playlist`,
                {
                    method: "POST",
                    headers: {
                        Authorization: `Bearer ${token}`,
    ...
    return (
        <aside
            ref={sidebarRef}
            className={`${
                isMobile ? (leftBar ? "show-left-bar" : "") : ""
            } sidebar-left ${expanded ? "expanded" : ""}`}
        >
            ...
            {!expanded ? (
                <div className="filters">
                    <button
                        className={`filter-btn ${
                            filter === "playlist" ? "active" : ""
                        }`}
                        onClick={() =>
                            setFilter(filter === "playlist" ? null : "playlist")
                        }
                    >
                        Playlists
                    </button>
                    <button
                        ...
                        onClick={() =>
                            setFilter(filter === "artist" ? null : "artist")
                        }
                    >
                        Artists
                    </button>
                    ...
                    <button
                        ...
                        onClick={() =>
                            setFilter(filter === "album" ? null : "album")
                        }
                    >
                        Albums
                    </button>
                ...
                <div className="filters">
                    ...
                    <div className="search-container" ref={searchRef}>
                        ...
                        <input
                            type="text"
                            className={`library-search expanded-input ${
            ...
            {expanded ? (
                ...
                <>
                    <div className="search-icon-row" ref={searchRef}>
                        ...
                        <input
                            type="text"
                            className={`library-search collapsed ${
                                showSearchBox ? "active" : ""
                            }`}
                            placeholder="Search"
                            value={searchTerm}
